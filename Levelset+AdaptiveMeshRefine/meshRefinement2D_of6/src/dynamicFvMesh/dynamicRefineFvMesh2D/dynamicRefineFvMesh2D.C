...
) const
{
    ...
    forAll(splitEdges, j)
    {
        ...
        forAll(e, i)
        {
            ...
            // What is the meaning of unrefineLevel?
...
//LC updated to avoid wedges to be excluded from refinement
void Foam::dynamicRefineFvMesh2D::checkMaxAnchorPoints
(
...
) const
{
    ...
    forAll(pointLevel, pointi)
    {
        ...
        forAll(pCells, pCelli)
        {
            ...
            {
                // Check if hex cell has already 8 anchor points -> protect cell
                ...
                // Check if wedge cell has already 6 anchor points -> protect cell
    ...
    {
        if (!protectedCell[celli] && cellPoints(celli).size() == 8 && nAnchorPoints[celli] != 8)
        {
            protectedCell.set(celli, true);
            nProtected++;
        }

        if (!protectedCell[celli] && cellPoints(celli).size() == 6 && nAnchorPoints[celli] != 6)
        {
            protectedCell.set(celli, true);
            nProtected++;
        }
    ...
    protectedCell_(nCells(), 0)
{
...

    {
        ...
        // LC: Also protect any cells that are less than hex or wedge
        forAll(cells(), celli)
        {
            const cell& cFaces = cells()[celli];
            ...
            // LC avoid to put wedges in protected list
        ...
        // LC: Check cells for corner points
        checkMaxAnchorPoints(protectedCell_, nProtected);
